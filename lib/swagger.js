...
SwaggerModel.prototype.setReferencedModels = function(allModels) {
    ...
    for (var i = 0; i < this.properties.length; i++) {
        ...
        if (allModels[type] != null)
          ...
          if(typeof property.refDataType == 'object') {
            for(var j in property.refDataType) {
              if(allModels[property.refDataType[j]] != null)
                results.push(property.refModel = allModels[property.refDataType[j]]);
            }
          } else if(allModels[property.refDataType] != null) {
...
SwaggerModel.prototype.getMockSignature = function(modelsToIgnore, dropReadOnly) {
  var propertiesStr = [];
  for (var i = 0; i < this.properties.length; i++) {
    prop = this.properties[i];
    if(dropReadOnly && prop.readOnly) continue;
...
var SwaggerModelProperty = function(name, obj) {
  ...
  this.patternProperties = obj.patternProperties;
  ...
  } else if (obj.patternProperties != null) {
    var self = this;
    for(var i in obj.patternProperties) {
      if(obj.patternProperties[i].$ref) {
        if(!self.refDataType) self.refDataType = {};

        self.refDataType[i] = obj.patternProperties[i].$ref;
      }
    }
  }
...
SwaggerModelProperty.prototype.getSampleValue = function(modelsToIgnore) {
  ...
  if ((this.refModel != null) && (modelsToIgnore.indexOf(prop.refModel.name) === -1)) {
    if(typeof this.refDataType == 'object') {
      result = {};
      for(var i in this.refDataType) {
        result[i] = this.refModel.createJSONSample(modelsToIgnore);
      }
    } else {
...
SwaggerModelProperty.prototype.toString = function() {
  ...
  var str = '<span class="propName ' + req + '">' + this.name + '</span>';

  if(this.patternProperties) {
    str += '(<span class="propType">';
    for(var i in this.patternProperties) {
        str += '<div class="propPattern">"' + i + '" : ' + (this.patternProperties[i].$ref ? this.patternProperties[i].$ref : this.patternProperties[i].type)  + '</div>';
    }
    str += ')</span>';
  } else {
    str += '(<span class="propType">' + this.dataTypeWithRef + '</span>';
  }
  ...
  if (this.readOnly) {
    str += ', <span class="propImmutableKey">immutable</span>';
  }
  str += ')';
  if (this.values != null) {
...
var SwaggerOperation = function(nickname, path, method, parameters, summary, notes, type, responseMessages, resource, consumes, produces, authorizations) {
  ...
  for(var i = 0; i < this.parameters.length; i ++) {
    ...
    param.signature = this.getSignature(type, this.resource.models, true);
...
SwaggerOperation.prototype.getSignature = function(type, models, dropReadOnly) {
  ...
  if (isPrimitive) {
    ...
    if (listType != null) {
      return models[listType].getMockSignature(undefined, dropReadOnly);
    } else {
      return models[type].getMockSignature(undefined, dropReadOnly);
    }
  }
};
